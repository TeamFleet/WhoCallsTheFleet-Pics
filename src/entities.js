...
const convert = async (id) => {
    ...
    if (!fs.existsSync(fileAvatarJPG)) {
        ...
        await new Promise((resolve, reject) => {
            fs.writeFile(fileAvatarJPG, binaryData, 'binary', (err) => {
                if (err) reject(err);
                console.log(`  │           2.jpg`);
                resolve();
            });
    ...
    if (!fs.existsSync(fileAvatarThumbnail)) {
        for (let maskname of masks) {
            ...
            await new Promise((resolve, reject) => {
                exec(
                    `gm composite -geometry ${geometry} -compose in "${fileAvatarJPG}" ${fileMask} ${filePNG}`,
                    (err) => {
                        if (err) {
                            console.error(err);
                            return reject(err);
...
const run = async () => {
    ...
    getDb('entities').forEach((data) => {
        ...
        fs.ensureDirSync(path.join(dirEntities, '' + data.id));
    });

    for (let id of fs
        .readdirSync(dirEntities)
